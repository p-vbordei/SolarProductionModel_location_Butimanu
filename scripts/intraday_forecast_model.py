...
class IntradaySolarForecastModel:
    ...
    def __init__(self, location_key: str, location_config: Dict):
        ...
        # DC capacity (from panels) for DC power calculations
        self.dc_capacity_mw = location_config.get('dc_capacity_mw', location_config['estimated_capacity_mw'])
        self.dc_capacity_kw = self.dc_capacity_mw * 1000

        # AC capacity (from inverters) - use ac_limit_mw if available (operational limit)
        # Otherwise fall back to ac_capacity_mw (nameplate) or estimated_capacity_mw
        self.ac_capacity_mw = location_config.get('ac_limit_mw',
                                                  location_config.get('ac_capacity_mw',
                                                                     location_config['estimated_capacity_mw']))
        self.ac_capacity_kw = self.ac_capacity_mw * 1000

        # Legacy capacity field for backward compatibility
        self.capacity_mw = self.ac_capacity_mw
        self.capacity_kw = self.ac_capacity_kw
    ...
    def predict_intraday(self, weather_df: pd.DataFrame, 
                        current_conditions: Optional[Dict] = None) -> pd.DataFrame:
        """
        ...
        predictions['energy_mwh'] = predictions['production_kw'] / 1000 * resolution_hours
        predictions['energy_q10_mwh'] = predictions['q10'] / 1000 * resolution_hours
        predictions['energy_q25_mwh'] = predictions['q25'] / 1000 * resolution_hours
        predictions['energy_q50_mwh'] = predictions['q50'] / 1000 * resolution_hours
        predictions['energy_q75_mwh'] = predictions['q75'] / 1000 * resolution_hours
        predictions['energy_q90_mwh'] = predictions['q90'] / 1000 * resolution_hours
    ...
    def _calculate_dc_power(self, weather_df: pd.DataFrame, 
        ...
        # Use DC capacity from panels (not AC capacity)
        ...
        return dc_power.clip(lower=0, upper=self.dc_capacity_mw)  # Never exceed DC nameplate capacity
    
    def _apply_system_losses(self, dc_power: pd.Series, 
                           weather_df: pd.DataFrame) -> pd.Series:
        """Apply system losses and convert to AC power"""
        ...
        # Calculate load ratio relative to AC capacity
        load_ratio = dc_power / (self.ac_capacity_mw + 0.001)
        ...
        # Clip to AC inverter capacity (DC power can exceed AC capacity due to oversizing)
    ...
    def _generate_uncertainty_bands(self, ac_power: pd.Series,
        ...
        # Convert MW to kW
        predictions['production_kw'] = ac_power * 1000
        ...
        # Generate quantiles (in kW)
        predictions['q10'] = ac_power * 1000 * (1 - 2 * total_uncertainty)
        predictions['q25'] = ac_power * 1000 * (1 - total_uncertainty)
        predictions['q50'] = ac_power * 1000  # Median = mean for this model
        predictions['q75'] = ac_power * 1000 * (1 + total_uncertainty)
        predictions['q90'] = ac_power * 1000 * (1 + 2 * total_uncertainty)

        # Ensure physical constraints
        for col in ['q10', 'q25', 'q50', 'q75', 'q90']:
            predictions[col] = predictions[col].clip(lower=0, upper=self.capacity_kw)
    ...
    def get_forecast_summary(self, predictions: pd.DataFrame) -> Dict:
        ...
        summary = {
            ...
            'peak_production_kw': predictions['production_kw'].max(),
            'total_energy_kwh': predictions['production_kw'].sum() * (INTRADAY_RESOLUTION_MINUTES / 60),
            'average_hourly_kw': predictions['production_kw'].mean(),
            'capacity_factor': predictions['production_kw'].mean() / self.capacity_kw,
            'generation_window': {
        ...
        producing = predictions[predictions['production_kw'] > 0.01]
        if not producing.empty:
            ...
            peak_idx = producing['production_kw'].idxmax()
