...
class EnhancedIntradayForecastingSystem:
    ...
    def run_single_forecast(self) -> Dict:
        ...
        try:
            ...
            if self.model_type == 'ml_physics':
                predictions_15min = self._run_ml_physics_model(weather_df, current_conditions)
                ...
                predictions_15min = self._run_comparison(weather_df, current_conditions)
            else:
            ...
            logger.info(f"Peak forecast: {summary_report['capacity_analysis']['peak_production_kw']:.1f} kW")
            logger.info(f"Total energy: {summary_report['energy_analysis']['total_energy_kwh']:.1f} kWh")
            logger.info("="*60)
    ...
    def _run_ml_physics_model(self, weather_df: pd.DataFrame, 
        ...
        predictions = self.ml_model.predict_intraday(weather_df, current_conditions)
    ...
    def _run_comparison(self, weather_df: pd.DataFrame, 
                       current_conditions: Dict) -> Dict[str, pd.DataFrame]:
        ...
        # Run ML/Physics model
        try:
            ml_predictions = self._run_ml_physics_model(weather_df, current_conditions)
    ...
    def get_forecast_history(self) -> Dict:
        ...
        for timestamp, forecast_data in self.forecast_history.items():
            history_data[timestamp] = {
                ...
                'peak_kw': forecast_data['summary']['capacity_analysis']['peak_production_kw'],
                'total_kwh': forecast_data['summary']['energy_analysis']['total_energy_kwh']
            }
    ...
    try:
        ...
        if args.mode == 'single':
            ...
            if result['status'] == 'success':
                ...
                print(f"📊 Peak production: {result['summary']['capacity_analysis']['peak_production_kw']:.1f} kW")
                print(f"⚡ Total energy: {result['summary']['energy_analysis']['total_energy_kwh']:.1f} kWh")
                
                if 'comparison' in result['summary']:
                    comp = result['summary']['comparison'].get('ml_vs_spm', {})
                    if comp:
                        print(f"\n📈 Model Comparison:")
                        print(f"   MAE: {comp.get('mae_kw', comp.get('mae_mw', 0)):.1f} kW")
                        print(f"   RMSE: {comp.get('rmse_kw', comp.get('rmse_mw', 0)):.1f} kW")
