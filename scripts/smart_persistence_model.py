...
class SmartPersistenceModel:
    ...
    def __init__(self, location_key: str, location_config: Dict):
        ...
        # AC capacity (from inverters) for final output clipping
    ...
    def _calculate_clear_sky_pvlib(self, timestamp: datetime) -> float:
        ...
        try:
            ...
            # Convert GHI to power using DC capacity
            # Simple model: Power = DC Capacity * (GHI / 1000) * Performance Ratio
            clear_sky_power = self.dc_capacity_mw * (ghi / 1000) * self.performance_ratio

            # Clip to AC capacity (inverter limit)
            return float(min(clear_sky_power, self.ac_capacity_mw))
    ...
    def _calculate_clear_sky_simple(self, timestamp: datetime) -> float:
        ...
        # Convert to power using DC capacity
        clear_sky_power = self.dc_capacity_mw * (ghi / 1000) * self.performance_ratio
